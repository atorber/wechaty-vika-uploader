...
async function onLogin(user: Contact) {
    ...
    if (isUpdate) {
        ...
        for (let i = 0; i < rooms.length; i++) {
            ...
            console.debug(roomsRecord.length)
            if (roomsRecord.length == 5) {
        ...
        for (let i = 0; i < contactList.length; i++) {
            ...
            console.debug(contactsRecord.length, i)
            ...
            if (contactsRecord.length == (i + 1)) {
                await vika.createRecords(vika.sheets.Room.id, contactsRecord)
            }
        }
    }
